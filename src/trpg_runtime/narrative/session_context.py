...
RULING_RULES = (
    ...
    "可行与不可行都必须给出 world_response：\n"
    "- 可行时，world_response 描述世界如何回应这个行动，包含真实后果。\n"
    "- 不可行时，world_response 仍然是叙事的一部分，让玩家从世界里明白为什么行不通。"
    "不要写「系统拒绝」「该行为不允许」这类跳出故事的话。\n"
    ...
    "tension_resolved 表示当前这段张力是否已经被解决。只有确实解决了才置为 true；"
