...
async def _complete_text(
    ...
    for attempt in range(3):
        ...
        try:
            raw = await author.complete_text(
                current, temperature=0.2, max_tokens=max_tokens
            )
            last_raw = str(raw or "")
            text = last_raw.strip()
            if text:
...
def _source_plan_prompt(
    ...
    schema = {
        ...
        "excluded_ranges": [
            {"start_line": 1, "end_line": 10, "reason": "metadata or license"}
        ],
        "chapters": [
            {
                "ordinal": 1,
...
async def _prepare_source_document(
    ...
    *,
    ...
    document = _materialise_source_document(
        source_path, raw, source_id, title, locale, plan
    )
    manifest = workspace.load_manifest()
    if rebuild:
        manifest = {}
...
def _world_book(bundle: StoryBundle, world_document: str) -> dict[str, Any]:
    ...
    return {
        "name": bundle.title,
        "description": (
            "Generated by TARI source decomposition; source references remain "
            "in the Story Bundle."
        ),
        "scan_depth": 4,
        "entries": entries,
...
async def compile_document(
    ...
    *,
    ...
    timeout_only_compatible = isinstance(stored_settings, dict) and (
    ...
    manifest.setdefault("stages", {}).update(
        {"structures": True, "novel_outline": True}
    )
    workspace.save_manifest(manifest)
